...
class PyKinectRuntime(object):
    ...
    def __init__(self, frame_source_types):
        ...
        #Changed:{
        self._long_exposure_infrared_source = self._sensor.LongExposureInfraredFrameSource
        self._long_exposure_infrared_frame_desc = self._long_exposure_infrared_source.FrameDescription
        #Changed:}
        ...
        #Changed:{
        if(self.frame_source_types & FrameSourceTypes_LongExposureInfrared):
            self._long_exposure_infrared_frame_data = ctypes.POINTER(ctypes.c_ushort) 
            self._long_exposure_infrared_frame_data_capacity = ctypes.c_uint(self.infrared_frame_desc.Width * self.infrared_frame_desc.Height)
            self._long_exposure_infrared_frame_data_type = ctypes.c_ushort * self._long_exposure_infrared_frame_data_capacity.value
            self._long_exposure_infrared_frame_data = ctypes.cast(self._long_exposure_infrared_frame_data_type(), ctypes.POINTER(ctypes.c_ushort))
            self._long_exposure_infrared_frame_reader = self._long_exposure_infrared_source.OpenReader()
            self._long_exposure_infrared_frame_arrived_event = self._long_exposure_infrared_frame_reader.SubscribeFrameArrived()
            self._handles[self._waitHandleCount] = self._long_exposure_infrared_frame_arrived_event
            self._waitHandleCount += 1
        #Changed:}
    ...
    def get_last_infrared_frame(self):
        with self._infrared_frame_lock:
            if self._infrared_frame_data is not None:
                ...
                _last_infrared_frame_access = time.clock()
                return data
            else:
                return None
    #Changed:}
    ...
    #Changed:{
    def get_last_long_exposure_infrared_frame(self):
        with self._long_exposure_infrared_frame_lock:
            if self._long_exposure_infrared_frame_data is not None:
                data = numpy.copy(numpy.ctypeslib.as_array(self._long_exposure_infrared_frame_data, shape=(self._long_exposure_infrared_frame_data_capacity.value,)))
                _last_long_exposure_infrared_frame_access = time.clock()
                return data
            else:
                return None
    #Changed:}
    ...
    def handle_long_exposure_infrared_arrived(self, handle_index):
        longExposureInfraredFrameEventData = self._long_exposure_infrared_frame_reader.GetFrameArrivedEventData(self._handles[handle_index])
        longExposureInfraredFrameRef =longExposureInfraredFrameEventData.FrameReference
        try:
            longExposureInfraredFrame =longExposureInfraredFrameRef.AcquireFrame()
            try:
                with self._long_exposure_infrared_frame_lock:
                    longExposureInfraredFrame.CopyFrameDataToArray(self._long_exposure_infrared_frame_data_capacity, self._long_exposure_infrared_frame_data)
                    self._last_long_exposure_infrared_frame_time = time.clock()
            except: 
                pass
            longExposureInfraredFrame = None
        except:
            pass
        longExposureInfraredFrameRef = None
        longExposureInfraredFrameEventData = None 
    #Changed:}
